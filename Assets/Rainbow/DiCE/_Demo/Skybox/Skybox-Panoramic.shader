...
Shader "DiCE/Skybox/Panoramic" {													// Modified for DiCE
Properties {
...
SubShader {
    ...
    Pass {
...
#endif
            UNITY_VERTEX_OUTPUT_STEREO												// Required for DiCE, but in most Unity shaders already
			float4 screenPos : TEXCOORD6;											// Added for DiCE
        };
...
#endif
			o.screenPos = ComputeScreenPos(o.vertex);								// Added for DiCE
            return o;
        }
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            c *= _Exposure;
			UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);							// Added for DiCE
			c = GetDiceCol(c, i.screenPos.x / i.screenPos.w);						// Added for DiCE
            return half4(c, 1);
        }
        ENDCG
    }
