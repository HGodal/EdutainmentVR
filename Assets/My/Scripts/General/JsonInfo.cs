using System.Collections.Generic;

[System.Serializable]
public class JsonInfo
{
    public string toolSorter1;
    public string toolSorter2;
    public string toolSorter3;

    public string pipeBuilder1;
    public string pipeBuilder2;

    public string pipeFixer1;
    public string pipeFixer2;

    public List<string> ventilationSteps;
    public string insideVentilationRules;
    public string outsideVentilationRules;

    public List<string> quizList;
    public string quiz1;
    public string quiz2;

    public string ventSetup;
    public string ventSetup1;
    public string ventSetup2;
    public string ventSetup3;

    public string ventFixer1;
    public string ventFixer2;
    public string ventFixer3;
    public string ventFixer4;
    public string ventFixer5;

}
